(function () {
  ...
  */
  function calculator(operator) {
    If(!isOperatorValid(operator))//if(true)
    {
      return false;
    }
    return function (x, y) {
      if (typeof x !== 'number' || typeof y !== 'number') {//se x ou y for inválido return false
        return false;
      }
      return operation[operator](x, y);//objeto operation [indcie operador] função conforme operação
    };
  }
